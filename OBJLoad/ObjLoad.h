...
#include "../PBR_d12/GeometryGenerator.h"


typedef GeometryGenerator::MeshData Mesh;
...
namespace MeshTool {
	...
	inline Mesh convertToMeshdata(const ObjModel &obj);
	...
	inline Mesh convertToMeshdata(const ObjModel& obj)
	{
		...
		for (std::vector<ObjModel::FaceVertex>::const_iterator f = unique.begin(); f != unique.end(); ++f) {
			...
			float x = (obj.vertex.begin() + 3 * f->v)->;
